	...
	public static void main(String[] args) throws MalformedURLException, RemoteException, NotBoundException, InterruptedException
	{
		...
		try{
			...
			if(input>3 || input<1)
			{
				...
				if(input == 1)
				{
					System.out.println("Player 2 has selected Rock.");
				}
				else if(input == 2)
				{
					System.out.println("Player 2 has selected Paper.");
				}
				else if(input == 3){
					System.out.println("Player 2 has selected Scizzor.");
				}
			}
		...
		rem.registerForCallback("Player2", input,remClient);
